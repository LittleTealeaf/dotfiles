...
src="$DOT_FILES/bin/project_info"
dest="$HOME/.local/bin/project_info"

mkdir -p "$(dirname "$dest")"

if [ -e "$dest" ] || [ -L "$dest" ]; then
    if [ -L "$dest" ] && [ "$(readlink -f "$dest")" = "$(readlink -f "$src")" ]; then
        echo -e "  [${GREEN}Linked${NC}] $dest already points to bin/project_info"
        exit 0
    fi
    backup="${dest}.bak"
...
ln -s "$src" "$dest"
echo -e "  [${GREEN}Created${NC}] Symlinked $dest -> bin/project_info"
