...
def signal_handler(sig, frame):
    logger.info("Recieved signal to stop, exiting")
    sys.stdout.write("\n")
    sys.stdout.flush()
    sys.exit(0)
...
class PlayerManager:
    ...
    def on_player_appeared(self, _, player):
        ...
        if player is not None and (self.selected_player is None
                                   or player.name == self.selected_player):
            self.init_player(player)
        else:
            logger.debug(
